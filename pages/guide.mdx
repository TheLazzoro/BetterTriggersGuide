...
You can work directly in external text/code editors without the need to copy-paste the contents into the editor like we are used to in the World Editor.
...
/>
<br/>
This feature is exclusive to <code>.j</code> and <code>.lua</code> files.
External changes to trigger and variable files will not be picked up by the editor, but it's generally not recommended
